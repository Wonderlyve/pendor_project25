...
export const useOptimizedPostsFixed = () => {
  ...
  const fetchPosts = useCallback(async (pageNum: number, limit: number = POSTS_PER_PAGE) => {
    ...
    try {
      ...
      if (user && transformedPosts.length > 0) {
        const postIds = transformedPosts.map(post => post.id).filter(id => id && id !== 'undefined');
  ...
  const likePost = async (postId: string) => {
    ...
    // Validation de l'ID avant d'envoyer la requête
    ...
    try {
      ...
      if (checkError && checkError.code !== 'PGRST116') {
        ...
        toast.error('Erreur lors de la vérification du statut de like');
        return;
      }
